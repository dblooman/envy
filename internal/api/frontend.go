...
func (h *handler) frontend(w http.ResponseWriter,r *http.Request) {
	...
	if !ok { writeError(w,&domain.Error{Code:"unavailable",Message:"frontend bindings are unavailable"}); return }
	...
	case "PUT /v1/projects/{project}/frontend-bindings/{frontend}/{revision}":
		var req domain.BindFrontendRequest; if !decodeCatalog(w,r,&req) { return }; result,err=s.BindFrontend(r.Context(),k,req)
	...
	case "POST /v1/projects/{project}/frontend-bindings/{frontend}/{revision}/deployment":
		var req domain.PublishFrontendRequest; if !decodeCatalog(w,r,&req) { return }; result,err=s.PublishFrontend(r.Context(),k,req)
	case "POST /v1/projects/{project}/frontend-bindings/{frontend}/{revision}/check":
		var req domain.FrontendCheckRequest; if !decodeCatalog(w,r,&req) { return }; result,err=s.CheckFrontend(r.Context(),k,req)
	case "GET /v1/compositions/{id}/frontend-bindings":
		after,limit,pageErr:=pagination(r); if pageErr!=nil { writeError(w,pageErr); return }
		var items []domain.FrontendBindingView; var next string
		items,next,err=s.FrontendBindings(r.Context(),r.PathValue("id"),after,limit)
		result=map[string]any{"items":items,"next_cursor":next}
	}
	if err!=nil { writeError(w,err); return }; writeJSON(w,http.StatusOK,result)
}
