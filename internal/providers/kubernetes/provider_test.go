...
func TestMessagingBindingsReplaceBaselineEnvironment(t *testing.T) {
	for _, isolated := range []bool{false, true} {
		p, k, s := fixture()
		ctx := context.Background()
		s.Profile.Env = map[string]string{"SUB": "baseline-subscription"}
		value := ""
		if isolated {
			value = "preview-subscription"
		}

		s.MessagingEnv = map[string]string{"SUB": value, "ENVY_MESSAGE_ISOLATION": fmt.Sprint(isolated)}
		ref, err := p.Ensure(ctx, s)
		if err != nil {
			t.Fatal(err)
		}

		d, _ := k.AppsV1().Deployments(ref.Namespace).Get(ctx, ref.Deployment, metav1.GetOptions{})
		count := 0
		for _, e := range d.Spec.Template.Spec.Containers[0].Env {
			if e.Name == "SUB" {
				count++
				if e.Value != value || e.ValueFrom != nil {
					t.Fatal("baseline binding retained")
				}
			}
		}

		if count != 1 {
			t.Fatal("duplicate or absent binding")
		}
	}
}
