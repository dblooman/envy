...
func TestBaselineMeshParticipation(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.mesh+tc.proxy, func(t *testing.T) {
			k := fake.NewSimpleClientset(&core.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "baseline"}}, &core.Service{ObjectMeta: metav1.ObjectMeta{Name: "api", Namespace: "baseline"}, Spec: core.ServiceSpec{Selector: map[string]string{"app": "api"}, Ports: []core.ServicePort{{Name: "http", Port: 8080, Protocol: core.ProtocolTCP}}}}, &core.Pod{ObjectMeta: metav1.ObjectMeta{Name: "api", Namespace: "baseline", Labels: map[string]string{"app": "api"}}, Spec: core.PodSpec{Containers: []core.Container{{Name: "api"}}}, Status: core.PodStatus{Conditions: []core.PodCondition{{Type: core.PodReady, Status: core.ConditionTrue}}, ContainerStatuses: []core.ContainerStatus{{Name: tc.proxy, Ready: true}}}})
