...
func TestChainPropagatesRequestComposition(t *testing.T) {
	...
	defer g.Close()
	r, _ := http.NewRequest(http.MethodGet, g.URL+"/api/offer", nil)
...
func TestMiddleServiceSelectsOfferByVersion(t *testing.T) {
	...
	Handler("service-a", "", "a", "baseline").ServeHTTP(baseline, httptest.NewRequest(http.MethodGet, "/api/offer", nil))
	...
	Handler("service-a", "", "a", "preview").ServeHTTP(preview, httptest.NewRequest(http.MethodGet, "/api/offer", nil))
