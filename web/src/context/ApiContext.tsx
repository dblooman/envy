...
interface ApiContextType {
  ...
  createComposition: (req: CreateCompositionRequest, idempotencyKey?: string) => Promise<Composition>
  updateComposition: (id: string, req: UpdateCompositionRequest) => Promise<Composition>
...
export function ApiProvider({ children }: { children: React.ReactNode }) {
  ...
  const [compositions, setCompositions] = useState<Composition[]>(INITIAL_MOCK_COMPOSITIONS)
    ...
    try {
      ...
      if (res.status === 'ok') {
        try {
          ...
          return { ok: true, message: 'Connected to Envy Control Plane with authenticated access.' }
          ...
          if (authMsg.includes('unauthorized') || authMsg.includes('401')) {
            return {
              ok: false,
              message: 'Server online, but Bearer Token is invalid or missing. Please set your API token.',
            }
          }
  ...
  const refreshAll = useCallback(async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to fetch from Envy server'
      setError(msg)
    } finally {
      setLoading(false)
    }
  ...
  const createComposition = async (
    ...
    if (isDemoMode) {
      ...
      const newComp: Composition = {
        ...
        conditions: [
          { type: 'WorkloadsReady', status: false, message: 'Creating pod override' },
          { type: 'RoutesConfigured', status: true, message: 'Istio VirtualService baggage rule created' },
          { type: 'RouteVerified', status: false, message: 'Synthetic HTTP probe pending' },
        ],
        latest_operation: {
          id: `op-${Date.now()}`,
          kind: 'create',
          ...
          prev.map((c) => {
            if (c.id === newId) {
              return {
                ...
                conditions: [
                  { type: 'WorkloadsReady', status: true, message: 'All override pods running and healthy' },
                  { type: 'RoutesConfigured', status: true, message: 'VirtualService routing active' },
                  { type: 'RouteVerified', status: true, message: 'Verified 200 OK via ingress' },
                ],
                latest_operation: {
                  ...c.latest_operation,
                  status: 'completed',
              ...
              return {
                ...
                components: {
                  ...
                  'service-b': {
                    ...
                    image: req.overrides['service-b']?.image || c.components['service-b'].image,
                  },
                },
                latest_operation: {
                  ...c.latest_operation,
