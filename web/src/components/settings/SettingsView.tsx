    ...
    <div className="max-w-3xl space-y-6">
      ...
      <Card className="border-border bg-card shadow-2xs">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Switch between Crisp Daylight (Light) and Refined Neutral Slate (Dark).
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-3 gap-3">
            {[
              { id: 'light', label: 'Daylight Light', icon: Sun, desc: 'Clean slate & white canvas' },
              { id: 'dark', label: 'Refined Dark', icon: Moon, desc: 'Neutral graphite slate' },
              { id: 'system', label: 'System Auto', icon: Laptop, desc: 'Follow OS preference' },
            ].map((opt) => {
              const Icon = opt.icon
        ...
        <CardContent className="text-xs text-muted-foreground">
          {isDemoMode ? (
            <div className="p-3 rounded-lg bg-sky-50 border border-sky-200 text-sky-900 dark:bg-sky-950/40 dark:border-sky-900 dark:text-sky-300 flex items-center gap-2">
              ...
              <span>
                Demo mode is currently <strong>Active</strong>. You can create, update, and destroy preview compositions safely.
              </span>
            </div>
          ) : (
            <div className="p-3 rounded-lg bg-muted/60 border border-border text-foreground flex items-center gap-2">
              ...
              <span>
                Live Mode is active. API requests are directed to the Envy server at{' '}
        ...
        <form onSubmit={handleSave}>
          <CardHeader>
            ...
            <CardDescription>
              ...
              <code className="font-mono text-foreground font-semibold">:8081</code>.
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="space-y-4 text-xs sm:text-sm">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-foreground">Server Base URL</label>
              <Input
                value={inputUrl}
