...
for (const [name, definition] of Object.entries(diagrams)) {
  test(`${name}: every connection is routed without crossing a service card`, async () => {
      ...
      for (const section of edge.sections ?? []) {
        ...
        for (let index = 1; index < points.length; index++) {
          ...
          for (const node of nodes) {
            const x = node.x!, y = node.y!, right = x + node.width!, bottom = y + node.height!;
            const verticalCross = a.x === b.x && a.x > x && a.x < right && Math.max(a.y, b.y) > y && Math.min(a.y, b.y) < bottom;
            const horizontalCross = a.y === b.y && a.y > y && a.y < bottom && Math.max(a.x, b.x) > x && Math.min(a.x, b.x) < right;
            assert.ok(!verticalCross && !horizontalCross, `${edge.id} crosses ${node.id}`);
          }
        }
      }
    }
    for (const node of nodes) {
      ...
      assert.ok(node.x! + node.width! <= graph.width! && node.y! + node.height! <= graph.height!);
    }
  });
}
