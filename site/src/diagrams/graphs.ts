...
export type DiagramNode = { id: string; title: string; detail: string; kind: Kind; label: string };
export type DiagramEdge = { from: string; to: string; label: string; kind?: Kind; feedback?: boolean };
export type Diagram = { title: string; description: string; nodes: DiagramNode[]; edges: DiagramEdge[] };

const node = (id: string, title: string, detail: string, kind: Kind, label: string): DiagramNode => ({ id, title, detail, kind, label });
const edge = (from: string, to: string, label: string, kind: Kind = 'control', feedback = false): DiagramEdge => ({ from, to, label, kind, feedback });

const services = [
  node('gateway', 'gateway · v1', 'Shared entry service', 'shared', 'Baseline workload'),
  node('a', 'service-a · v1', 'Forwards request baggage', 'shared', 'Baseline workload'),
  node('b1', 'service-b · v1', 'Baseline namespace', 'shared', 'Baseline workload'),
  node('b2', 'service-b · v2', 'Composition namespace', 'override', 'Override workload'),
  node('data', 'Database & cache', 'Preview writes affect this data', 'shared', 'Shared dependencies'),
];
...
export const diagrams: Record<string, Diagram> = {
  ...
  control: {
    ...
    nodes: [
      ...
      node('api', 'Envy REST API', 'Validate and accept intent', 'control', 'Control plane'),
  ...
  workflow: {
    ...
    nodes: [
      ...
      node('cleanup', 'Destroy & report', 'destroy_composition', 'control', '05 · Envy MCP + agent'),
    ],
    edges: [edge('build', 'create', 'Published image'), edge('create', 'wait', 'Composition ID'), edge('wait', 'test', 'Ready URL'), edge('test', 'logs', 'Checks fail', 'action'), edge('logs', 'build', 'Build a new revision', 'action', true), edge('test', 'cleanup', 'Testing complete')],
  },
  'agent-summary': {
    title: 'An agent’s preview loop',
    description: 'Build an image, wait for a ready composition, test its URL, then request cleanup. If a test fails, inspect logs and repeat with a new image.',
    nodes: [
      node('image', 'Build image', 'Agent or existing CI workflow', 'action', '01 · Prepare'),
      node('preview', 'Create & wait', 'Envy API through MCP', 'control', '02 · Provision'),
      node('test', 'Test preview URL', 'HTTP checks or browser tests', 'action', '03 · Verify'),
      node('done', 'Destroy & report', 'Record the actual test results', 'control', '04 · Finish'),
    ],
    edges: [edge('image', 'preview', 'Image reference'), edge('preview', 'test', 'Ready URL'), edge('test', 'done', 'Testing complete')],
  },
  frontend: {
    title: 'Connect a frontend to a backend preview',
    description: 'Bind an exact frontend commit to a composition. The build adapter resolves the ready backend URL, passes it to the frontend build, and records the hosting URL. Browser tests call that backend through its preview ingress.',
    nodes: [node('revision', 'Frontend commit', 'Exact Git revision', 'action', 'Source'), node('binding', 'Frontend binding', 'Revision → composition ID', 'control', 'Envy API'), node('build', 'Frontend build', 'Receives resolved API URL', 'action', 'Build adapter'), node('browser', 'Deployed frontend', 'Browser calls the preview API', 'action', 'External hosting'), node('preview', 'Preview ingress', 'Sets composition context', 'override', 'Backend composition'), services[0], services[1], services[3]],
    edges: [edge('revision', 'binding', 'Bind revision'), edge('binding', 'build', 'Resolve ready API URL'), edge('build', 'browser', 'Deploy assets'), edge('browser', 'preview', 'API request', 'override'), edge('preview', 'gateway', 'Preview request', 'override'), edge('gateway', 'a', 'Forward baggage', 'shared'), edge('a', 'b2', 'Matching context', 'override')],
  },
};
