...
export function layoutDiagram(name: string, direction: 'DOWN' | 'RIGHT') {
  ...
  if (!cache.has(key)) {
    ...
    if (ids.size !== diagram.nodes.length) throw new Error(`Duplicate nodes in ${name}`);
    for (const edge of diagram.edges) {
      if (!ids.has(edge.from) || !ids.has(edge.to)) throw new Error(`Unconnected edge in ${name}`);
    }
    cache.set(key, elk.layout({
      id: key,
      layoutOptions: {
      ...
      children: diagram.nodes.map(node => ({ id: node.id, width: 212, height: 82 })),
      edges: diagram.edges.map((edge, index) => ({
        id: `edge-${index}`, sources: [edge.from], targets: [edge.to],
        labels: [{ text: edge.label, width: edge.label.length * 6.5 + 16, height: 24,
