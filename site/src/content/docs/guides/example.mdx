...
3. **Downstream Mesh Route**: When `service-a` calls `service-b`, the Istio mesh VirtualService inspects the `baggage: composition=cmp-84f1a` header and redirects the call to `service-b-v2`.
