...
Applications must extract baggage from incoming requests and inject it into downstream requests. Envoy sidecars route using the resulting header; they cannot propagate context between separate application requests. Standard OpenTelemetry instrumentation can handle propagation without an Envy-specific SDK. See the [complete HTTP example](/reference/routing/#context-propagation).

---

## Resiliency Guarantees

- **Request execution**: Envy’s control plane is entirely out-of-band. Once Istio routes are configured, application traffic flows directly through Envoy proxies.
