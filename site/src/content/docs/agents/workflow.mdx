...
1. Decide whether a running preview is needed. Reuse an explicitly associated
   preview when suitable; inspect its generation and expiry. Call `list_components`
   for project `shop` before creating an override and select exact published builds
   when available. Use `overrides={}` when the complete baseline is sufficient.
...
5. Send the complete desired override set on updates, including earlier changes
   you need to retain. Updates keep the URL and do not renew TTL. Choose message
   isolation explicitly at creation for an onboarded Pub/Sub application.
6. Destroy disposable previews after verification. If the user requested a review
   environment, report its URL and expiry and retain it for that purpose.
```
...
```bash
envy recipe export cmp-7f39a > recipe.json
```
...
envy recipe recreate --file recipe.json \
