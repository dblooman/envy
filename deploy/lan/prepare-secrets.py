...
def main():
    ...
    namespace = {
        "apiVersion": "v1",
        "kind": "Namespace",
        "metadata": {"name": "envy-system"},
    }
    subprocess.run(
        kube + ["apply", "-f", "-"], input=json.dumps(namespace).encode(), check=True
    )
    ...
    def stored(name, secret_name):
        ...
        if not path.exists():
            ...
            if existing.strip():
                raise SystemExit(
                    f"Restore the original {path} before reinstalling; existing {secret_name} will not be rotated"
                )
    ...
    password, token = (
        stored("postgres-password", "envy-database"),
        stored("api-token", "envy-machines"),
    )
    ...
    if (
        set(registry) != {"auths"}
        or set(registry["auths"]) != {"ghcr.io"}
        or not registry["auths"]["ghcr.io"].get("auth")
    ):
    ...
    records = [
        (
            ...
            {
                "password": password,
                "url": f"postgres://envy:{password}@envy-postgres.envy-system.svc.cluster.local:5432/envy?sslmode=disable",
            },
        ),
        (
            "envy-machines",
            "Opaque",
            {
                "credentials.json": json.dumps(
                    [
                        {
                            "id": "lan-client",
                            "display_name": "LAN acceptance client",
                            "token": token,
                        }
                    ]
                )
        ...
        (
            ...
            {".dockerconfigjson": json.dumps(registry)},
        ),
    ]
    for name, kind, data in records:
        ...
        obj = {
            "apiVersion": "v1",
            "kind": "Secret",
            "metadata": {"name": name, "namespace": "envy-system"},
            "type": kind,
            "stringData": data,
        }
        ...
        if result.returncode:
            raise SystemExit(
                f"Could not create {name}; inspect cluster access (credential payload omitted)"
            )
    print(
        "Secret setup complete. Retained Secrets may differ from local files; use the original API token if this is a reinstall."
    )
