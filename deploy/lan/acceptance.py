...
class Client:
    ...
    def call(self, origin, method, path, headers=None, body=None):
        ...
        request = urllib.request.Request(origin + path, data=data, headers=headers or {}, method=method)
        if data is not None:
        ...
        with response:
            ...
            try:
                ...
                body = None
            return response.status, dict((k.lower(), v) for k, v in response.headers.items()), body
    ...
    def traffic(self, host):
        return self.call(self.ingress, "GET", "/products", {"Host": host, "baggage": "composition=hostile,tenant=test"})
...
def accept(client, builds, report, timeout=180):
    ...
    status, _, _ = client.call(client.api, "GET", "/v1/session", {"Authorization": "Bearer invalid"})
    if status != 401:
    ...
    report["baseline"] = {"body": baseline_observation[2], "storefront_workload": original[0], "pricing_workload": original[1]}
    ...
    try:
        ...
        intent = {"project": "lan", "baseline": "staging", "name": "branch-pricing", "ttl": "10m", "overrides": {"pricing": {"image": builds["pricing_branch"]["image"]}}}
        ...
        def baseline():
            if check_chain(client.traffic(baseline_host), "", baseline_price) != original:
        ...
        def wait(phase):
            ...
            while time.monotonic() < deadline:
                ...
                if current["phase"] == "failed" and phase != "destroyed":
                    raise RuntimeError("composition failed; inspect its status through Envy")
                time.sleep(1)
        ...
        if not endpoint["ready"] or url.hostname != "cmp-" + composition["id"] + ".envy.test" or url.port != 30080:
        ...
        for _ in range(5):
            ...
            baseline()
            observations.append({"body": observation[2], "storefront_workload": preview[0], "pricing_workload": preview[1], "storefront_context": observation[1].get("x-shop-storefront-context"), "pricing_context": observation[1].get("x-shop-pricing-context")})
        ...
        report["cluster_checks"] = "pending: inspect owned namespace absence and retained PostgreSQL on the cluster Mac"
    finally:
        if composition and not destroyed:
            try:
                client.api_call("DELETE", "/v1/compositions/" + composition["id"], expected=202)
                report["cleanup"] = "deletion requested; inspect until destroyed"
            except Exception:
                report["cleanup"] = "deletion request failed; inspect composition before TTL expires"
...
def main():
    ...
    report = {"result": "failed", "api": args.api, "ingress": args.ingress, "builds": builds, "started_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}
    start = time.monotonic()
    try:
        accept(Client(args.api, args.ingress, Path(args.token_file).read_text().strip()), builds, report)
