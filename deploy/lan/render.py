...
def load_builds(path):
    ...
    for name in ("storefront_main", "pricing_main", "pricing_branch"):
        item = builds[name]
        repo = (
            "envy-test-storefront" if name == "storefront_main" else "envy-test-pricing"
        )
        if not re.fullmatch(
            r"ghcr.io/dblooman/" + repo + r"@sha256:[a-f0-9]{64}", item["image"]
        ):
            raise ValueError(f"invalid immutable image for {name}")
        if (
            not re.fullmatch(r"[a-f0-9]{40}", item["revision"])
            or not str(item["run_id"]).isdigit()
        ):
            raise ValueError(f"missing source/run provenance for {name}")
    if (
        builds["pricing_main"]["revision"] == builds["pricing_branch"]["revision"]
        or builds["pricing_main"]["image"] == builds["pricing_branch"]["image"]
    ):
...
def render(builds):
    ...
    objects = [
        {
            "apiVersion": "v1",
            "kind": "Namespace",
            "metadata": {"name": ns, "labels": {"istio-injection": "enabled"}},
        }
    ]
    catalog = {
        "api_version": "envy/v1",
        "project": {"id": "lan", "name": "LAN APIs"},
        "components": [],
        "baseline": {
            "id": "staging",
            "project": "lan",
            "revision": "main-"
            + builds["storefront_main"]["revision"][:12]
            + "-"
            + builds["pricing_main"]["revision"][:12],
            "endpoint": "http://baseline.envy.test:30080",
            "routing": {
                "namespace": ns,
    ...
    for name in ("storefront", "pricing"):
        ...
        if name == "storefront":
            env["DOWNSTREAM_URL"] = (
                f"http://pricing.{ns}.svc.cluster.local:8080/products"
            )
                ...
                "spec": {
                    ...
                    "template": {
                        ...
                        "spec": {
                            ...
                            "imagePullSecrets": [{"name": "envy-ghcr"}],
        ...
        catalog["components"].append(
            {
                ...
                "overridable": name == "pricing",
                "env": env,
                "image_pull_secrets": ["envy-ghcr"],
            }
        )
        catalog["baseline"]["components"][name] = {
    ...
    objects += [
        {
            ...
            "kind": "Gateway",
            "metadata": {"name": "envy-preview", "namespace": ns},
            "spec": {
        ...
        },
        {
            "apiVersion": "networking.istio.io/v1",
            "kind": "VirtualService",
            "metadata": {"name": "baseline", "namespace": ns},
            "spec": {
                "hosts": ["baseline.envy.test"],
                ...
                "http": [
                    {
                        ...
                        "route": [
                            {
                                "destination": {
                                    "host": f"storefront.{ns}.svc.cluster.local",
                                    "port": {"number": 8080},
                                }
                            }
                        ],
