...
class LANTests(unittest.TestCase):
    def test_catalog_and_workloads_share_immutable_bindings(self):
        ...
        self.assertEqual(
            [c["id"] for c in catalog["components"] if c["overridable"]], ["pricing"]
        )

    def test_context_and_business_changes_are_required(self):
        headers = {
            "x-shop-storefront-context": "cmp",
            "x-shop-pricing-context": "cmp",
            "x-shop-storefront-workload": "main-a",
            "x-shop-pricing-workload": "branch-b",
        }
        ...
        dropped = copy.deepcopy(headers)
        dropped["x-shop-pricing-context"] = ""
        with self.assertRaises(RuntimeError):
    ...
    def test_chart_http_and_tls_preflight(self):
        ...
        tls = subprocess.check_output(
            ...
            + [
                "--set",
                "runtime.previewBaseURL=https://envy.test:8443",
                "--set",
                "runtime.ingressURL=https://ingress:9443",
                "--set",
                "runtime.caConfigMap.name=private-ca",
            ],
            text=True,
        )
    ...
    def exercise(self, wrong_branch):
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def composition(self):
                return {
                    "id": composition_id,
                    "phase": "destroyed" if state["destroyed"] else "ready",
                    "endpoints": {
                        "public": {
                            "ready": True,
                            "url": "http://cmp-abc123.envy.test:30080",
                        }
                    },
            ...
            def do_POST(self):
                ...
                if (
                    body["overrides"]["pricing"]["image"]
                    != builds["pricing_branch"]["image"]
                ):
                    self.reply(400, {})
                    return
                state["created"] += 1
            ...
            def do_GET(self):
                if self.path.startswith("/v1/"):
                    ...
                    self.reply(
                        200,
                        self.composition()
                        if "/compositions/" in self.path
                        else {"principal": {"id": "lan-client", "kind": "service"}},
                    )
                    return
                ...
                if host not in (
                    "baseline.envy.test:30080",
                    "cmp-abc123.envy.test:30080",
                ) or (preview and state["destroyed"]):
                    self.reply(404, {})
                    return
                context = composition_id if preview else ""
                headers = {
                    "X-Shop-Storefront-Context": context,
                    "X-Shop-Pricing-Context": context,
                    "X-Shop-Storefront-Workload": "main-a",
                    "X-Shop-Pricing-Workload": "branch-b" if preview else "main-b",
                }
        ...
        try:
            ...
            if wrong_branch:
                with self.assertRaises(RuntimeError):
                    accept(
                        Client(origin, origin, "test-token"), builds, report, timeout=5
                    )
            ...
            self.assertEqual(
                state["created"], 2
            )  # One intent, retried with the same key.
        finally:
            server.shutdown()
            server.server_close()
            worker.join()
