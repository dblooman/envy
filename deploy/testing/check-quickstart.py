...
command = ["helm", "template", "envy", chart, "--namespace", "envy-quickstart", "--include-crds"]
rendered = subprocess.check_output(command, text=True)
assert 'helm.sh/hook' not in rendered, "Bundled DB must migrate through an init container"
...
standard = subprocess.check_output(["helm", "template", "envy", "deploy/helm/envy", "--set", "installationID=test"], text=True)
