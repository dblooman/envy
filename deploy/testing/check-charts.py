...
for provider in ["istio", "cilium", "linkerd"]:
    cmd = [
        ...
        chart,
        "--set",
        "installationID=chart-test",
        "--set",
        "mesh.provider=" + provider,
    ]
    if provider != "istio":
        cmd += ["--set", "runtime.ingressURL=https://gateway.example.test"]
    ...
    if provider != "istio":
        assert "istio-ingressgateway" not in rendered
    for doc in rendered.split("---"):
        if "config.json: |-" in doc:
            config = json.loads(doc.split("config.json: |-")[1])
            assert config["mesh"]["provider"] == provider
