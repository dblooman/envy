...
root=pathlib.Path(__file__).resolve().parents[2];state=pathlib.Path(os.environ['ENVY_STATE_DIR'])
base=os.environ['ENVY_API_URL'];token=(state/'api-token').read_text().strip()
...
def traffic(url,composition,leaf):
 ...
 args=['curl','--fail','--silent','--show-error','--max-time','10','--resolve',f'{u.hostname}:{u.port}:127.0.0.1',url]
 if u.scheme=='https':args.extend(['--cacert',str(state/'preview.crt')])
...
legacy=subprocess.check_output(['kubectl','exec','-n','envy-system','deployment/postgres','--','psql','-U','envy','-d','envy','-Atc',"SELECT to_regclass('installation_profile') IS NULL"],text=True).strip()
assert legacy=='t','upgrade fixture must begin without provider-binding metadata'
c=request('/v1/compositions','POST',{'project':'demo','baseline':'staging','name':'upgrade-smoke','overrides':{'service-b':{'image':'envy/service-b:v2'}},'ttl':'20m'})
id=c['id'];before=wait(id,'ready');endpoint=before['endpoints']['public']['url']
ns='envy-'+id
inventory=subprocess.check_output(['kubectl','get','deployment/service-b','service/service-b','-n',ns,'-o','json'],text=True)
...
after=wait(id,'ready');assert after['endpoints']['public']['url']==endpoint
inventory=json.loads(subprocess.check_output(['kubectl','get','deployment/service-b','service/service-b','-n',ns,'-o','json'],text=True))
assert {x['kind']:x['metadata']['uid'] for x in inventory['items']}==uids,'upgrade replaced workload identity'
traffic(endpoint,id,'v2')
