...
def call(host, baggage=()):
    ...
    try:
        ...
        for value in baggage: c.putheader('baggage',value)
        c.endheaders()
        r=c.getresponse(); body=r.read()
        return r.status,json.loads(body) if r.status==200 else body.decode(errors='replace')
    finally: c.close()
...
def chain(host,version,composition,baggage=()):
    ...
    assert all(h['composition']==composition and h['workload_id'] for h in hops),result
    assert [h['deployment_composition'] for h in hops]==['baseline','baseline',composition or 'baseline'],result
    return result
...
def eventually(check,timeout=90):
    ...
    while True:
        try: return check()
            ...
            if time.monotonic()>=end: raise AssertionError(f'{stage} deadline: {e}') from e
            time.sleep(.5)

def baseline():
    value=chain('baseline.envy.localhost','v1','',('composition=spike','composition=hostile,tenant=outside'))
    if stage!='before': assert value==json.loads((state/'spike-baseline.json').read_text()),value
    return value
...
if stage=='before':
    ...
    def active():
        value=chain('cmp-spike.envy.localhost','v2','spike',('composition=hostile','tenant=outside,composition=another'))
        base=baseline()
        assert [h['workload_id'] for h in value['chain'][:2]]==[h['workload_id'] for h in base['chain'][:2]],value
        assert value['chain'][2]['workload_id']!=base['chain'][2]['workload_id'],value
    ...
    eventually(absent)
else: raise SystemExit('unknown spike stage')
