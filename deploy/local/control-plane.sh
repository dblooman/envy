...
for name in ['api-token','postgres-password']:
 p=root/name
...
kubectl -n envy-system create secret generic envy-credentials --from-file=api-token="$ENVY_STATE_DIR/api-token" --from-file=postgres-password="$ENVY_STATE_DIR/postgres-password" --from-file=database-url="$ENVY_STATE_DIR/database-url" --dry-run=client -o yaml | kubectl apply -f -
