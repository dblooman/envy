...
class PrerequisiteTests(unittest.TestCase):
    def test_missing_docker_has_actionable_failure(self):
        ...
        import io
        stderr = io.StringIO()
        with patch('sys.argv', ['doctor', '--local']), patch.dict('os.environ', {}, clear=True), patch.object(doctor.shutil, 'which', return_value=None), patch.object(doctor, 'port_available', return_value=True), contextlib.redirect_stderr(stderr):
    ...
    def test_invalid_ports_fail_before_cluster_creation(self):
        ...
        import io
        stderr = io.StringIO()
        with patch('sys.argv', ['doctor', '--local']), patch.dict('os.environ', {'ENVY_API_PORT': 'invalid'}, clear=True), patch.object(doctor.shutil, 'which', return_value=None), patch.object(doctor, 'port_available', return_value=True), contextlib.redirect_stderr(stderr):
            self.assertEqual(doctor.main(), 1)
