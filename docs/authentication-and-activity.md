...
Set `ENVY_AUTH_MODE` to `token` (the compatible default), `proxy`, or `none`. Token mode requires at least one shared or named machine credential; a shared token is not required when named credentials are configured. Token mode accepts the existing `ENVY_API_TOKEN_FILE` shared credential and named machine credentials from `ENVY_MACHINE_CREDENTIALS_FILE`. The machine file is a JSON array of `{"id":"release-agent","display_name":"Release agent","token":"..."}` objects. Tokens must be unique and at least 32 characters. The existing build-credential file remains restricted to its repository build-report route.
