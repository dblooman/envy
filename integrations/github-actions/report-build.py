...
def report(file, environment=os.environ):
    ...
    try:
        ...
        raise ValueError(
            f"Envy rejected the build report (HTTP {exc.code}); check repository scope, image availability, and report identity"
        ) from None
    except urllib.error.URLError:
        raise ValueError(
            "Envy could not be reached; retry using the same report file"
        ) from None
...
def main():
    ...
    parser.add_argument(
        "--file", required=True, help="JSON build report; reuse unchanged when retrying"
    )
