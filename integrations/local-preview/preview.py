...
def validate_report(report, run, run_id, component):
    ...
    actual = (str(report['run_id']), report['attempt'], report['revision'], report['component'])
    if actual != expected:
        raise RuntimeError('Artifact does not match the selected run, attempt, commit and component')
...
def wait(api, cid, target, timeout):
    ...
    while True:
        ...
        if current['phase'] == target:
            return current
        if target == 'ready' and current['phase'] in ('failed', 'destroying', 'destroyed'):
            raise RuntimeError('Composition %s is %s; inspect it in Envy' % (cid, current['phase']))
        if time.monotonic() >= deadline:
            raise RuntimeError('Timed out waiting for %s; composition %s remains %s' % (target, cid, current['phase']))
        time.sleep(2)
...
def run(args):
    ...
    with (state / 'workflow.lock').open('w') as lock:
        try:
            ...
            raise RuntimeError('This workflow is already running (including its frontend server)') from None
        ...
        identity = {k: config[k] for k in ('api_url', 'project', 'baseline', 'repository', 'github_repository', 'component', 'name')}
        ...
        if command(['git', 'status', '--porcelain'], cwd=frontend).strip():
            raise RuntimeError('Commit or stash frontend changes before binding an exact revision')
        if len(revision) != 40:
            raise RuntimeError('Frontend requires a full Git SHA')
        # Reserve the browser port before creating or updating anything.
        server = None
        if not args.no_serve:
            server = http.server.ThreadingHTTPServer(('127.0.0.1', int(config.get('port', 4174))),
                functools.partial(http.server.SimpleHTTPRequestHandler, directory=str(frontend / 'dist')))
        try:
            if session and not session.get('composition'):
                # Recover the exact accepted intent even if CI has since been rerun.
                recovered = api.call('POST', '/v1/compositions', session['create_request'],
                    {'Idempotency-Key': session['idempotency_key']})
                session['composition'] = recovered['id']
                save(session_path, session)
            repo = config['github_repository']
            run_info = json.loads(command(['gh', 'api', 'repos/%s/actions/runs/%s' % (repo, args.run)]))
            if run_info['conclusion'] != 'success':
                raise RuntimeError('Wait for the selected Actions run to succeed, then retry')
            with tempfile.TemporaryDirectory(dir=state) as download:
                command(['gh', 'run', 'download', args.run, '--repo', repo,
                         '--name', 'envy-build-report', '--dir', download])
                report = json.loads((Path(download) / 'envy-build-report.json').read_text())
            ...
            build = reporter.call('POST', '/v1/projects/%s/repositories/%s/builds' % (project, repository), report)
            ...
            if session.get('composition'):
                ...
                if current['phase'] in ('destroying', 'destroyed'):
                    raise RuntimeError('Saved composition is deleted; use a new state_dir for a new preview')
                wanted = update_overrides(current, config['component'], build['id'])
                existing = {k: {('build_id' if v.get('build_id') else 'image'):
                    v.get('build_id') or v['image']} for k, v in current['overrides'].items()}
                if wanted != existing:
                    current = api.call('PATCH', '/v1/compositions/' + current['id'],
                        {'expected_generation': current['generation'], 'overrides': wanted})
            else:
                # Save an intent key before POST so retries after a crash cannot duplicate creation.
                if not session:
                    import uuid
                    session = {'identity': identity, 'idempotency_key': str(uuid.uuid4()), 'create_request': {
                        'project': config['project'], 'baseline': config['baseline'], 'name': config['name'],
                        'overrides': {config['component']: {'build_id': build['id']}}, 'ttl': config.get('ttl', '8h'),
                    }}
                    save(session_path, session)
                current = api.call('POST', '/v1/compositions', session['create_request'],
                    {'Idempotency-Key': session['idempotency_key']})
            ...
            path = '/v1/projects/%s/frontend-bindings/%s/%s' % (project, urllib.parse.quote(frontend_name, safe=''), revision)
            api.call('PUT', path, {'composition': cid, 'repository': config['frontend_repository']})
            ...
            result = {'composition': cid, 'generation': current['generation'], 'build': build['id'],
                      'api_url': receipt['api_url'], 'frontend': frontend_name, 'revision': revision,
                      'expires_at': receipt['expires_at']}
            if server:
                url = 'http://localhost:%s' % server.server_port
                api.call('POST', path + '/deployment', {'expected_version': receipt['binding_version'], 'url': url})
                result['frontend_url'] = url
            save(state / 'result.json', result)
            print(json.dumps(result), flush=True)
            if server:
                print('Serving frontend; Ctrl-C stops the server and retains the preview. Use --destroy to clean up.', file=sys.stderr, flush=True)
...
def main():
    ...
    parser.add_argument('--run', help='explicit successful Actions run ID (also used for rollback)')
    parser.add_argument('--no-serve', action='store_true', help='build without starting or publishing the local frontend')
    parser.add_argument('--destroy', action='store_true', help='destroy the saved composition and wait for cleanup')
    parser.add_argument('--timeout', type=int, default=120, choices=range(1, 601), metavar='SECONDS')
