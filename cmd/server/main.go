...
func run(parent context.Context) error {
	...
	go func() {
		...
		lead(ctx, store, kube, istio, verifier, installation, fileConfig.Istio.InjectionLabels, fileConfig.Istio.IngressSelector, fileConfig.ApprovedImagePullSecrets, fileConfig.Preview, reconciler.Config{Interval: interval, ProvisionTimeout: provision, DrainTimeout: drain})
	}()
...
func lead(ctx context.Context, store *postgres.Store, kube kubernetes.Interface, istio istioclient.Interface, verifier *verification.Demo, installation string, injectionLabels, ingressSelector map[string]string, approvedPullSecrets []string, previewPolicy kubeprovider.PreviewPolicy, cfg reconciler.Config) {
	for ctx.Err() == nil {
		...
		if err == nil {
			...
			worker := reconciler.New(store, kubeprovider.NewWithInjection(kube, installation, guard, injectionLabels).WithApprovedPullSecrets(approvedPullSecrets).WithPreviewPolicy(previewPolicy), istioprovider.NewWithIngressSelector(istio, installation, guard, ingressSelector), verifier, guard, slog.Default(), cfg)
